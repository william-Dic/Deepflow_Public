...
class BrowserContextConfig:
	...
	Default values:
		...
		device_scale_factor: float | None = None
			Optional scaling factor for small screens.
			A value >1 will cause the page to render zoomed-out so that more content is visible.
	"""
...
class BrowserContext:
	...
	async def _create_context(self, browser: PlaywrightBrowser):
		...
		if self.browser.config.cdp_url and len(browser.contexts) > 0:
			...
			# Updated: include device_scale_factor so that extra scaling can be applied.
			context = await browser.new_context(
				...
				device_scale_factor=self.config.device_scale_factor if self.config.device_scale_factor is not None else 1,
			)
